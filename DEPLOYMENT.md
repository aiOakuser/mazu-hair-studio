...
**Target:** `mazu-hair-studio.beauty.globaldesignerhub.com`, a tenant
...
```text
/var/www/mazu-hair-studio/     # repo checkout, built in place
```

## 2. systemd service

Unit file: `/etc/systemd/system/mazu-hair-studio.service`
...
WorkingDirectory=/var/www/mazu-hair-studio
...
Config lives at `/etc/nginx/sites-available/mazu` on the box (SSL
...
server {
    ...
    server_name mazu-hair-studio.beauty.globaldesignerhub.com;
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/mazu-hair-studio.beauty.globaldesignerhub.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/mazu-hair-studio.beauty.globaldesignerhub.com/privkey.pem;
...
`mazu-hair-studio.beauty.globaldesignerhub.com` resolves (Cloudflare) to
...
that happens: `sudo chown -R www-data:www-data /var/www/mazu-hair-studio`
...
```bash
curl -I https://mazu-hair-studio.beauty.globaldesignerhub.com
sudo systemctl status mazu-hair-studio
```
